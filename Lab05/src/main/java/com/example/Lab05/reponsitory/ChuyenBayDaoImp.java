...
import com.example.Lab05.entity.ChuyenBay;

@Repository
public interface ChuyenBayDaoImp extends CrudRepository<ChuyenBay, String> {
	...
	@Query(value = "SELECT * FROM chuyenbay WHERE chuyenbay.DoDai < ( SELECT MAX(maybay.TamBay) FROM maybay WHERE maybay.Loai LIKE (%:loai%) )",nativeQuery = true)
	public List<String> findMaxTamBayOfBoeing(@Param(value = "loai")String loai);
